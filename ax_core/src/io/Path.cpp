...
ax_string	Path::GetDirName( const ax_string & path ) {
	auto sz = path.c_str();
	auto p  = ax_strrchr_list( sz, ax_sz("\\/") );
	if( !p ) return ax_string::Make_c_str( sz );

	return ax_string::Make( sz, p-sz );
}
...
ax_string	Path::GetExt	( const ax_string	& path ) {
	auto sz = path.c_str();
...
ax_string	Path::ChangeExt	( const ax_string & path, const ax_string & new_extension ) {
	auto sz = path.c_str();
	auto p = ax_strrchr( sz,(ax_char)'.');
	if( !p ) {
		return path + new_extension;
	}else{
		return ax_string::Make( sz, p-sz+1 );
	}
}
